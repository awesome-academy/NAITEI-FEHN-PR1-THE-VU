    ...
    const fetchNews = async () => {
      try {
        ...
        const [news, categories] = await Promise.all([
          fetch("http://localhost:5000/news?_sort=-created_at").then((res) =>
            res.json(),
          ),
          fetch("http://localhost:5000/categories").then((res) => res.json()),
        ]);

        const outStandingNews = news
          .slice()
      ...
      <div className="mb-20">
        ...
        <ul className="text-sm">
          {categories.map((category, id) => (
            <li
              key={id}
              className="hover:text-hover cursor-pointer mb-1.5 flex items-center justify-start border-b border-gray-400 pb-1.5"
            >
              <span className="mr-1">
        ...
        <ul>
          {outstandingNews.map((news) => (
            <li key={news.id}>
              <a
                href={`/new/${news.id}`}
                className="flex items-start mb-2 bg-gray-50 rounded"
              >
                <img
                  src={news.cover}
                  alt=""
                  className="size-14 mr-2 rounded-l"
                />
                <p className="text-[0.65rem] font-beVietNamPro mt-0.5 text-gray-900 hover:text-hover">
                  {news.title}
                </p>
              </a>
            </li>
